...
function initRoomManageTool() {
	room_manage_tool = {
		init: function() {
			$.ajax({
				...
				success : function (data, response, status) {
					$("#roomTypeObj_typeId_query").combobox({ 
					    valueField:"typeId",
					    textField:"typeName",
					    panelHeight: "200px",
					...
					$("#roomTypeObj_typeId_query").combobox("loadData",data); 
				}
			});
		},
		reload : function () {
		...
		search: function() {
			...
			queryParams["roomTypeObj.typeId"] = $("#roomTypeObj_typeId_query").combobox("getValue");
